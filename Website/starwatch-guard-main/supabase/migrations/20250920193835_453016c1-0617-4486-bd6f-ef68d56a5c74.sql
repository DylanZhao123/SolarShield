...
-- Comprehensive space weather cache table
...
CREATE TABLE IF NOT EXISTS public.visualization_configs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Create trigger for visualization configs
DROP TRIGGER IF EXISTS update_visualization_configs_updated_at ON public.visualization_configs;
CREATE TRIGGER update_visualization_configs_updated_at
    BEFORE UPDATE ON public.visualization_configs
    FOR EACH ROW
